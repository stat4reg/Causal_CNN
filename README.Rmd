...
Here the packages that are neccesary to running the the simulation are called.
```{r Packages}
library(DNNCausal)
library(tmle)
library(xtable)
...
The required functions that are used to perform the Post lasso estimators are loaded here:
```{r Post_lasso}
load('lasso_ATT.R')
```
...
## DGPs
Three different Data generating processes were considered in the simulation.
### First DGP
```{r DGP1}
...
```{r run_simulation}
sfinal = simulation_final(rep = 100, FUN =dat_gen_final_5_10)
